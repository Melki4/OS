...
void BarberWorker::run()
{
    while (true) {
        ...
        if (!socket.setSocketDescriptor(descriptor)) {
            qWarning() << "setSocketDescriptor failed:" << socket.errorString();
            emit clientFinished(m_id);
            continue;
        }

        int clientId = m_clientsServed + 1;
        socket.write((const char*)&clientId, sizeof(int)); socket.waitForBytesWritten();
        QByteArray name = ((m_id == 1) ? "[Парикмахер]" : "[Брат-мясник]");
        socket.write(name); socket.waitForBytesWritten();
        socket.write(m_greetings[0].arg(m_id).toUtf8()); socket.waitForBytesWritten();

        if (socket.waitForReadyRead(5000)) socket.readAll();

        QString menu = "Выберите стрижку:\n1. Кроп (1000)\n2. Фейд (1200)\n3. Налысо (500)\n";
        socket.write(menu.toUtf8()); socket.waitForBytesWritten();

        int choice = 0;
        if (socket.waitForReadyRead(30000)) socket.read((char*)&choice, sizeof(int));
        int price = (choice == 3) ? 500 : (choice == 2 ? 1200 : 1000);

        QStringList phrases = {"Начинаем стрижку...", "Так, получается хорошо...", "Готово! Отличный результат."};
        for (const QString &p : phrases) {
            socket.write(p.toUtf8()); socket.waitForBytesWritten(); QThread::sleep(1);
        }

        socket.write(m_farewells[0].arg(price).toUtf8()); socket.waitForBytesWritten();
        int method = 0;
        if (socket.waitForReadyRead(30000)) socket.read((char*)&method, sizeof(int));
        socket.write(m_farewells[1].toUtf8()); socket.waitForBytesWritten();
